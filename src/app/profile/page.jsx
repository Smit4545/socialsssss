...
export default async function Profile() {
  ...
  if (!userId) {
    return (
      <div className="flex justify-center items-center h-screen bg-gray-100">
        <p className="text-red-500 text-xl">User is not logged in. Please log in.</p>
      </div>
    );
  }

  const userRes = await fetch(`${process.env.URL}/api/users/${userId}`);
  const postsRes = await fetch(`${process.env.URL}/api/posts/user?userId=${userId}`);

  if (!userRes.ok || !postsRes.ok) {
    return (
      <div className="flex justify-center items-center h-screen bg-gray-100">
        <p className="text-red-500 text-xl">Error fetching profile data.</p>
      </div>
    );
  }
      ...
      <div className="max-w-5xl mx-auto">
        ...
        {posts?.length > 0 ? (
          ...
          <p className="text-gray-300 text-xl text-center mt-6">
            You have not made any posts yet. ✍️
          </p>
        )}
      </div>
    </div>
